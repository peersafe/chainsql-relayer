...
func (w *BoltDB) UpdateChainsqlHeight(h uint64) error {
	w.rwlock.Lock()
	defer w.rwlock.Unlock()
	...
	return w.db.Update(func(tx *bolt.Tx) error {
		bkt := tx.Bucket(BKTHeight)
		return bkt.Put([]byte("chainsql_height"), raw)
	})
}

func (w *BoltDB) GetChainsqlHeight() uint64 {
	w.rwlock.RLock()
	defer w.rwlock.RUnlock()
	...
	_ = w.db.View(func(tx *bolt.Tx) error {
		bkt := tx.Bucket(BKTHeight)
		raw := bkt.Get([]byte("chainsql_height"))
		if len(raw) == 0 {
			h = 0
			return nil
		}
